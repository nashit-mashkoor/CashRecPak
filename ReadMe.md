The project is inspired the work of [kshitizrimal](https://github.com/kshitizrimal/Cash-Recog-Project).
The aim like the original project is to help the visually impaired with the management of their curreny notes.
This repositry specifically aims to addres the problem in Pakistan.


# Requirements

## Core
- AI enabled identification of currency notes
- Real time detection of notes:
	- Type
	- Number
	- Total
- Voice enabled Feedback
## Extended
- Fraud Detection of Notes

# Break down 

## AI model

- [] Gather dataset online (Scrap)
- [] Capture dataset images for complex tasks
- [] Balanced dataset of around 3000 images 
- [] Label datasets using labellimg
- [] Train and Desgin Model using tensorflow:
	- [] Mobile Net
	- [] Yolov4
- [] Test model on computer
- [] Convert model to tensorflow lite
 

## Mobile App based UI

- [] Create basic UI
- [] Integrate AI model
- [] Enable Voice Interaction

